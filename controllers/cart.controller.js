...
class CartController {
  ...
  createCart = async (req, res, next) => {
   ...
   getCarts = async (req, res, next) => {
    ...
    try {
      ...
      if (cartsListUp.length === 0) {
        res.status(200).json({ message: "장바구니가 비어 있습니다." })
      } else {
  ...
  updateCart = async (req, res, next) => {
    ...
    try {
      const updateCartDate = await this.cartService.updateCt(cartsId, userId, quantity, );
  ...
  deleteCart = async (req, res, next) => {
    ...
    try {
      const deleteCartDate = await this.cartService.deleteCt(cartsId, userId);
